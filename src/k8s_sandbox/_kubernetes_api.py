...
def _get_client_refresh_seconds() -> int:
    raw = os.environ.get(INSPECT_K8S_CLIENT_REFRESH_SECONDS, "0")
    try:
        value = int(raw)
    except ValueError:
        raise ValueError(
            f"{INSPECT_K8S_CLIENT_REFRESH_SECONDS} must be a non-negative int: '{raw}'."
        )
    if value < 0:
        raise ValueError(
            f"{INSPECT_K8S_CLIENT_REFRESH_SECONDS} must be a non-negative int: '{value}'."
        )
    return value
...
class _ThreadLocalClientFactory:
    ...
    def _maybe_refresh(self, context_name: str | None) -> None:
        ...
        if old_client is not None:
            old_client.api_client.close()  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType]
        logger.info(
            "Refreshed Kubernetes client for context %s (age: %.0fs, max: %ds).",
            context_name or "(current)",
            age,
            refresh_seconds,
        )
